...
router.post("/facebook/getCompleteData", async (req, res) => {
  ...
  try {
    ...
    console.log(process.env.FRONTEND_HOST, queryParams);
    ...
    console.log("don2e", data1, queryParams1);
...
router.post("/facebook/getConversations", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ status: true, conversations: allConversations });
  } catch (error) {
    res.status(400).send({ status: false, error: error.message });
  }
});
...
router.post("/facebook/reloadConversations", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Conversations Reloaded!" });
  } catch (error) {
    res.status(400).send({ status: false, error: error.message });
  }
});
...
router.post("/facebook/sendMessage", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Message Sent!", data });
  } catch (error) {
    res.status(400).send({ status: false, error: error.message });
  }
});
